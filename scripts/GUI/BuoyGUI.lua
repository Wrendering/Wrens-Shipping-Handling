...
buoyGUI.handler_on_gui_opened = function (e)
  if e.entity and string.find(e.entity.name, "buoy%-entity") ~= nil then
    ...
    gui_auttab.add({type = "drop-down", name = "buoy_statepicker", items = buoyGUI.buoyTypesList, selected_index = global.lists["buoy-entity"][e.entity.unit_number].active_state })
  end
end
...
buoyGUI.stateHandler_on_gui_selection_state_changed = function(e)
  if e.element.valid and e.element.name == "buoy_statepicker" then
    ...
    local buoy = global.lists["buoy-entity"][buoy_unit_number]

    buoy.active_state = e.element.selected_index
    buoy = buoy:updateEntityWithChecks()

    gui_base.destroy()
    game.players[e.player_index].opened = buoy.entity
  end
end
